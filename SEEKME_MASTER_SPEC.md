...
- **ブランドの役割分担と用語規律**:
  - **本家 `ReMEETs`**: 【届ける再会】大切なあの人を指名してボトルメールを海に放つ（アクティブ） → **「手紙」「メール（ボトルメール）」**という言葉を使用。
  - **姉妹 `ReMEETs SEEKME`**: 【待つ・迎える再会】私を探している誰かに向けて、自分からのメッセージを届けておく（パッシブ・灯台型） → **必ず「メッセージ」**という言葉を使用（「手紙」は完全禁止）。

---

## 📜 2. 想い出のメッセージのデータ項目・表示設計（シンプル＆プライバシー厳守）

メッセージ（ボトル）の入力・表示項目は、ユーザーの手間を最小化し、プライバシーを厳格に保護するため**以下の5項目のみ**に絞り込みます。
...
メッセージを書いた側（Aさん）の負担をゼロにして投稿数を最大化しつつ、探した側（Bさん）が安心して決済・eKYCできる**相互合意フロー**です。

```mermaid
sequenceDiagram
    autonumber
    actor A as メッセージを書いた人 (Aさん)
    actor B as 探していた知人 (Bさん)
    
    Note over A: ① メッセージを投稿 (完全無料・SMS認証)
    Note over B: ② Google検索等でAさんのメッセージを発見
...
- **次回以降、他のメッセージへの申請や本家ReMEETsの利用時はeKYC手続き・費用（600円）が永久免除（無料パス）**となるため、無駄金になりません。
...
3. **ストーカー規制法・迷惑行為防止**:
   - メッセージの主（Aさん）に「ワンクリック拒絶・ブロック機能」を提供。
...
## 🎨 6. UI/UX・デザイン方針（メッセージページ＝完結型LP）

### (1) トップページ（ブランド＆実績ポータル）
- 掲示板のような雑多な一覧表示を完全排除し、サイトの品格を守る。
- **3大コンテンツ**:
  1. **実績カウンター**: 「海を漂う想い：〇〇通 ｜ 生まれた再会：〇〇組」
  2. **出会いの感謝のメッセージ**: 実際に再会したユーザーの温かい成功事例・エピソード
  3. **安心・安全の仕組み図解**: eKYC・相互合意・AI安全検閲の透明性解説

### (2) メッセージの個別ページ（最強のランディングページ）
Google検索から直接着地したユーザーのため、メッセージページ単体で以下の要素を1画面スクロールで提供。
1. **上部**: エモーショナルなメッセージデザイン（氏名、旧姓、ゆかりの都道府県、メッセージ）
2. **メインCTA**: 「この人に再会を希望する（エピソード送信）」ボタン
3. **安心解説**: 相互承認制・eKYC本人確認の仕組み図解
4. **サブCTA**: 「あなたも大切な人に向けてメッセージを書きませんか？（新規投稿）」
