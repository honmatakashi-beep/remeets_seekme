              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4 text-xs">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 bg-slate-50 border border-zinc-300 rounded-xl text-xs text-black focus:outline-none focus:border-sky-500 font-sans"
                    />
                  </div>
                      ...
                      <button
                        ...
                        className={`p-3 rounded-xl border text-center transition-all cursor-pointer ${
                          mypageEkycDocType === 'license'
                            ? 'border-sky-500 bg-sky-50 text-sky-900 font-bold ring-2 ring-sky-500/20'
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="flex-1 py-3 bg-gradient-to-r from-sky-500 via-sky-600 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white rounded-xl text-xs font-bold transition-all shadow-md flex items-center justify-center gap-1.5 cursor-pointer active:scale-98"
                  >
              ...
              <div className="space-y-6 font-sans">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="flex-1 py-3 bg-gradient-to-r from-sky-400 via-sky-500 to-blue-500 hover:from-sky-500 hover:to-blue-600 text-white rounded-xl text-xs font-bold transition-all shadow-md flex items-center justify-center gap-1.5 disabled:opacity-50 cursor-pointer active:scale-98"
                  >
                    {isMypagePaying ? (
                      <span>決済処理中...</span>
                    ) : (
              ...
              <div className="space-y-6 py-4 text-center font-serif">
                ...
                <div className="space-y-1.5 px-2">
                  <div className="flex items-center justify-between text-xs font-serif font-semibold text-zinc-500 px-1">
                    ...
                    <span className="text-sky-700 font-bold font-serif tracking-[0.12em]">{mypageEkycProgress} / 100%</span>
                  </div>
                ...
                <div className="bg-slate-50/80 rounded-xl p-3 border border-slate-200/60 text-left space-y-2 text-xs font-serif">
                  <div className={`flex items-center justify-between p-2 rounded-lg transition-all ${mypageEkycProgress >= 0 && mypageEkycProgress < 25 ? 'bg-white shadow-xs border border-sky-200 font-bold text-sky-900' : mypageEkycProgress >= 25 ? 'text-zinc-400 font-medium' : 'text-zinc-500'}`}>
                    <span className="flex items-center gap-2">
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-serif font-bold ${mypageEkycProgress >= 25 ? 'bg-sky-500 text-white' : 'bg-sky-100 text-sky-800'}`}>
                        {mypageEkycProgress >= 25 ? '✓' : '1'}
                      </span>
                      <span className="tracking-[0.08em] md:tracking-[0.12em]">決済承認＆セキュリティトークン化</span>
                    </span>
                    {mypageEkycProgress < 25 && <span className="text-[10px] text-sky-600 animate-pulse font-serif font-semibold tracking-[0.14em]">処理中...</span>}
                  </div>

                  <div className={`flex items-center justify-between p-2 rounded-lg transition-all ${mypageEkycProgress >= 25 && mypageEkycProgress < 50 ? 'bg-white shadow-xs border border-sky-200 font-bold text-sky-900' : mypageEkycProgress >= 50 ? 'text-zinc-400 font-medium' : 'text-zinc-500'}`}>
                    <span className="flex items-center gap-2">
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-serif font-bold ${mypageEkycProgress >= 50 ? 'bg-sky-500 text-white' : 'bg-sky-100 text-sky-800'}`}>
                        {mypageEkycProgress >= 50 ? '✓' : '2'}
                      </span>
                      <span className="tracking-[0.08em] md:tracking-[0.12em]">公的書類・文字データ暗号解析</span>
                    </span>
                    {mypageEkycProgress >= 25 && mypageEkycProgress < 50 && <span className="text-[10px] text-sky-600 animate-pulse font-serif font-semibold tracking-[0.14em]">解析中...</span>}
                  </div>

                  <div className={`flex items-center justify-between p-2 rounded-lg transition-all ${mypageEkycProgress >= 50 && mypageEkycProgress < 75 ? 'bg-white shadow-xs border border-sky-200 font-bold text-sky-900' : mypageEkycProgress >= 75 ? 'text-zinc-400 font-medium' : 'text-zinc-500'}`}>
                    <span className="flex items-center gap-2">
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-serif font-bold ${mypageEkycProgress >= 75 ? 'bg-sky-500 text-white' : 'bg-sky-100 text-sky-800'}`}>
                        {mypageEkycProgress >= 75 ? '✓' : '3'}
                      </span>
                      <span className="tracking-[0.08em] md:tracking-[0.12em]">実在生身人間（ライブネス）判定</span>
                    </span>
                    {mypageEkycProgress >= 50 && mypageEkycProgress < 75 && <span className="text-[10px] text-sky-600 animate-pulse font-serif font-semibold tracking-[0.14em]">判定中...</span>}
                  </div>

                  <div className={`flex items-center justify-between p-2 rounded-lg transition-all ${mypageEkycProgress >= 75 ? 'bg-white shadow-xs border border-sky-200 font-bold text-sky-900' : 'text-zinc-500'}`}>
                    <span className="flex items-center gap-2">
                      <span className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-serif font-bold ${mypageEkycProgress === 100 ? 'bg-sky-500 text-white' : 'bg-sky-100 text-sky-800'}`}>
                        {mypageEkycProgress === 100 ? '✓' : '4'}
                      </span>
                      <span className="tracking-[0.08em] md:tracking-[0.12em]">🛡️ 認証マーク付与＆本人確認完了</span>
                    </span>
                    {mypageEkycProgress >= 75 && mypageEkycProgress < 100 && <span className="text-[10px] text-sky-600 animate-pulse font-serif font-semibold tracking-[0.14em]">処理中...</span>}
                  </div>
                </div>

                {/* 動的精度スコア・解析テレメトリ詳細 */}
                <div className="bg-white/95 rounded-2xl p-3.5 border border-sky-200/80 shadow-2xs text-left space-y-2 font-sans">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    ...
                    <div className="bg-slate-50 p-2 rounded-xl border border-slate-100">
                      ...
                      <span className="font-mono font-bold text-sky-700 text-[10px] truncate block">
