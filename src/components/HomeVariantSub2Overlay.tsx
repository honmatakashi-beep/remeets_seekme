          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {(recentPosts && recentPosts.length > 0 ? recentPosts.slice(0, 4) : GALLERY_BOTTLES).map((b: any) => (
              <div 
                ...
                className="p-5 bg-white border-2 border-slate-200 hover:border-teal-500 rounded-3xl space-y-3 cursor-pointer text-left shadow-sm hover:shadow-md transition-all"
              >
                <div className="flex justify-between items-start">
                  <span className="text-[10px] font-bold text-teal-800 bg-teal-50 border border-teal-200 px-2.5 py-0.5 rounded-full">
                    {b.era || '思い出'} / {b.relationship || '大切な人'}
                  </span>
