...
import { Heart, Coffee, Sparkles, ChevronRight } from 'lucide-react';
    ...
    return (
      <>
        <button
          ...
          className={`inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-sky-600 via-teal-600 to-indigo-600 hover:from-sky-700 hover:to-indigo-700 text-white font-bold rounded-full text-xs shadow-xs transition-all hover:scale-[1.02] cursor-pointer ${className}`}
        >
    ...
    return (
      <>
        <div className={`p-4 md:p-5 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white rounded-2xl border border-indigo-900/60 shadow-lg flex flex-col md:flex-row items-center justify-between gap-4 font-sans ${className}`}>
          ...
          <button
            onClick={() => setIsModalOpen(true)}
            className="w-full md:w-auto shrink-0 px-5 py-2.5 bg-gradient-to-r from-sky-600 via-teal-600 to-indigo-600 hover:from-sky-700 hover:to-indigo-700 text-white font-bold rounded-xl text-xs transition-all shadow-md hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2 cursor-pointer whitespace-nowrap"
          >
            <Coffee size={15} className="text-white shrink-0" />
            <span>ReMEETsを応援（寄付）</span>
      ...
      <div className={`p-6 md:p-8 bg-gradient-to-br from-sky-50/90 via-teal-50/70 to-indigo-50/80 border-2 border-teal-200/90 rounded-3xl shadow-sm space-y-4 font-sans relative overflow-hidden ${className}`}>
        ...
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-teal-200/60 relative z-10">
          <div className="text-[11px] text-slate-500 flex items-center gap-1.5">
            ...
            <span>選択式（1口500円〜20口10,000円）／クレカ即時一括決済</span>
          </div>

          <button
            onClick={() => setIsModalOpen(true)}
            className="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-sky-600 via-teal-600 to-indigo-600 hover:from-sky-700 hover:to-indigo-700 text-white font-bold rounded-2xl text-xs md:text-sm transition-all shadow-md hover:shadow-lg hover:scale-[1.01] active:scale-95 flex items-center justify-center gap-2 cursor-pointer"
          >
