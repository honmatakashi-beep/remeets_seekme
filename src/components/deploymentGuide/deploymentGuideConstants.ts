export const POLICE_PRESENTATION_SCENARIOS = [
  ...
  "ユーザーの身元保証と安全な取引を担保するため、本人確認機関（TRUSTDOCK等）と決済インフラ（Stripe）を完全分離して連携する2社分離設計を採用。メッセージ開封時の電子的利用宣誓を必須とし、さらに公的証明を希望するユーザーには公的身分証による任意eKYC審査（600円）を提供。審査に【不合格】となった際は仮売上（オーソリ）が即座に自動全額返金されます。承認済ユーザーにのみ認証マークが点灯し、安心・公正な身元確認を実現します。",
